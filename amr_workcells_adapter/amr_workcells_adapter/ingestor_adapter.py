...
class IngestorHandle(Node):
    ...
    def __init__(self, config, nav_graph):
        ...
        self.ingestor_result_pub = self.create_publisher(
            IngestorResult, "/ingestor_results", 1
        )
    ...
    def handle_request_cb(self, msg: IngestorRequest):
        if self.request_guid != msg.request_guid:
            ...
            self.get_logger().info(
                f"Receive ingestor_requests: {msg.request_guid}, "
...
def main(argv=sys.argv):
    ...
    try:
        ingestor_handle.get_logger().info(
            "Beginning ingestor_manager node, shut down with CTRL-C"
        )
        executor.spin()
