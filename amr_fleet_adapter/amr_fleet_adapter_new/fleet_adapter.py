...
def main(argv=sys.argv):
            ...
            vertexs_config.update(
                {
                    str(vertex): VertexInfo(
                        vertexs_config[vertex][0], vertexs_config[vertex][1]
                    )
                }
            )
        else:
            vertexs_config.update(
                {str(vertex): VertexInfo(vertexs_config[vertex], False)}
            )
    ...
    for robot_name in fleet_config.known_robots:
        ...
        robots[robot_name] = RobotAdapter(
            robot_name, robot_config, node, api, fleet_handle, vertexs_config
        )
