...
class FleetConflictsHandle(Node):
    def __init__(self, config):
        ...
        prefix = 'http://' + self.config["rmf_fleet"]['fleet_manager']['ip'] + \
    ...
    def check_priority(self, A: int, B: int, robot_A: RobotState, robot_B: RobotState):
        if robot_A.battery_percent <= self.recharge_threshold:
            ...
            if len(robot_A.path):
                ...
                if self.dist(endPath_A, [robot_B.location.x, robot_B.location.y]) >= self.conflicts_distance:
                    return A
                else:
                    return B
            return B
            ...
            if len(robot_B.path):
                ...
                if self.dist(endPath_B, [robot_A.location.x, robot_A.location.y]) >= self.conflicts_distance:
                    return B
                else:
                    return A
            return A
    ...
    def fleet_states_cb(self, msg: FleetState):
        ...
        for i in range(dataLen):
            ...
            if (robot1Mode == RobotMode.MODE_IDLE or
                robot1Mode == RobotMode.MODE_CHARGING or
                # robot1Mode == RobotMode.MODE_PAUSED or
                robot1Mode == RobotMode.MODE_EMERGENCY or
                robot1Mode == RobotMode.MODE_REQUEST_ERROR):
                ...
                if len(self.robots[robot1Name].wait_LID) != 0:
                    for robot in self.robots[robot1Name].wait_LID:
                        # if self.api.resume(robot_name=robot, cmd_id=self.generate_task_id()):
                            self.mode_request(fleet_name=fleetName,
                                              robot_name=robot,
                                              mode=RobotMode.MODE_MOVING)
                            self.robots[robot].wait_HID = None
                            self.robots[robot].last_mode_request = None
                            self.get_logger().info(f"Robot: `{robot}` will resume from conflicts")
            ...
            for j in range(dataLen):
                ...
                if dataRobot[i].location.level_name == dataRobot[j].location.level_name:
                    posA = [dataRobot[i].location.x,
                            dataRobot[i].location.y]
                    posB = [dataRobot[j].location.x,
                            dataRobot[j].location.y]
                    ...
                    if self.dist(posA, posB) <= self.conflicts_distance:
                        ...
                        if (robot1Mode == RobotMode.MODE_MOVING):
                            ...
                            if (robot2Mode == RobotMode.MODE_MOVING):
                                ...
                                if self.check_priority(A=i,B=j,robot_A=dataRobot[i],robot_B=dataRobot[j]) == i:
                                    prioHID = i
                                    prioLID = j
                                else:
                                    prioHID = j
                                ...
                                if self.robots[dataRobot[prioLID].name].last_mode_request != RobotMode.MODE_PAUSED:
                                    # if self.api.pause(robot_name=dataRobot[prioLID].name, cmd_id=self.generate_task_id()):
                                        ...
                                        self.robots[dataRobot[prioLID].name].last_mode_request = RobotMode.MODE_PAUSED
                                        self.robots[dataRobot[prioLID].name].wait_HID = dataRobot[prioHID].name
                                        self.robots[dataRobot[prioHID].name].wait_LID.append(dataRobot[prioLID].name)
                                        self.get_logger().info(f"Robot `{dataRobot[prioLID].name}` will waiting"
                                                            f" `{dataRobot[prioHID].name}` for conflicts")                                     

                            # Nếu robot2 đang docking thì tạm dừng robot1
                            elif (robot2Mode == RobotMode.MODE_DOCKING):
                                if (self.robots[robot1Name].last_mode_request != RobotMode.MODE_PAUSED):
                                    # if self.api.pause(robot_name=robot1Name, cmd_id=self.generate_task_id()):
                                        self.mode_request(fleet_name=fleetName,
                                                          robot_name=robot1Name,
                                                          mode=RobotMode.MODE_PAUSED)
                                        self.robots[robot1Name].last_mode_request = RobotMode.MODE_PAUSED
                                        self.robots[robot1Name].wait_HID = robot2Name
                                        self.robots[robot2Name].wait_LID.append(robot1Name)
                                        self.get_logger().info(f"Robot `{robot1Name}` will waiting"
                            ...
                            # sẽ chuyển sang chế độ tạm dừng để tránh xung đột với wait_HID của robot2
                                ...
                                if (self.robots[robot2Name].wait_HID is not None and
                                    self.robots[robot2Name].wait_HID != robot1Name and
                                    self.robots[robot1Name].last_mode_request != RobotMode.MODE_PAUSED):
                                    # if self.api.pause(robot_name=robot1Name, cmd_id=self.generate_task_id()):
                                        self.mode_request(fleet_name=fleetName,
                                                          robot_name=robot1Name,
                        ...
                        if (self.robots[robot1Name].last_mode_request == RobotMode.MODE_PAUSED and
                            self.robots[robot1Name].wait_HID ==  robot2Name):
                            # if self.api.resume(robot_name=robot1Name, cmd_id=self.generate_task_id()):
                                ...
                                self.get_logger().info(f"Robot `{robot1Name}` will resume from conflicts")
